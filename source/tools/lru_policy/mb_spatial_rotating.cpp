...
/* touch a single byte to fault in the page */
static inline void touch_addr(volatile char *p) {
    volatile char v = *p;
    (void)v;
}

/* hammer SPATIAL_DEGREE offsets within page `pg` */
static void hammer_page(volatile char *region, long pg) {
...
static void maybe_print_progress(long done, long total, int *last_step) {
...
int main(int argc, char **argv) {
    ...
    /* Phase 0: fault in every page */
    for (long p = 0; p < rss_pages; ++p) {
        touch_addr(region + p * PAGE_SIZE);
    }
