...
#if defined(TARGET_LINUX) && defined(ICX)
// ICX fails to compile htons and ntohs macros on Linux, so we revert to function implementation in libc-dynamic
#undef htons
#undef ntohs
extern u_int16_t htons(u_int16_t x);
extern u_int16_t ntohs(u_int16_t x);
#endif
...
INT32 INTERACTIVE_LISTENER::OpenSocket()
{
    ...
    // Generate secure session id and write in the file
    int fd = open("/dev/random", O_RDONLY);
    ASSERT(fd > 0, "failed to open /dev/random " + decstr(errno));

    unsigned long long sid = 0;
    res                    = read(fd, &sid, sizeof(sid));
    ASSERT(res > 0, "failed to read random number");

    _sid_len = snprintf_s(_sid, sizeof(_sid), "%llu", sid);
    close(fd);

    // open the file with access permissions only to the user
    fd = open(_full_file.c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    ASSERT(fd > 0, "failed to open file " + decstr(errno));

    FILE* fp = fdopen(fd, "w");
    ASSERT(fp != NULL, "failed to open file " + decstr(errno));

    // writing the used port number and the sid to the file in Jason format.
    fprintf(fp, "{\"port\" : \"%d\",\n", ntohs(serv_addr.sin_port));
    fprintf(fp, " \"sid\" : \"%s\"}\n", _sid);
    fclose(fp);
...
VOID INTERACTIVE_LISTENER::MonitorFD(THREADID tid, CONTEXT* ctxt, SYSCALL_STANDARD std, VOID* v)
{
    ...
    if (syscall == SYS_close)
    {
        ...
        INT32 fd    = static_cast< INT32 >(arg);
...
VOID INTERACTIVE_LISTENER::WaitForUserSiganl(VOID* v)
{
    ...
    while (1)
    {
        ...
        {
            close(listener->_server_sock);
            unlink(listener->_full_file.c_str());
        ...
        else if (retval)
        {
            ...
            if (sessionSock >= 0)
            {
                ...
                int res = read(sessionSock, buf, sizeof(buf));
                if (res > 0)
                {
                    unsigned int buf_len = strnlen_s(buf, sizeof(buf));
                    // Sometimes listener returns one extra byte
                    if ((buf_len == listener->_sid_len || buf_len == listener->_sid_len - 1) &&
                        strncmp(listener->_sid, buf, buf_len) == 0)
                    {
                    ...
                    else if (++tries >= 3)
                    {
                        ...
                        close(listener->_server_sock);
                        unlink(listener->_full_file.c_str());
                        ASSERT(FALSE, "received the wrong session id");
                    }
                }
                close(sessionSock);
            }
